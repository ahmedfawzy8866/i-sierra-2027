...
/** Lightweight card shape for the public-facing property list view. */
...
/** @deprecated Use PropertyCard. Kept for legacy import compatibility. */
export type Property = PropertyCard;

/**
 * Fetches properties from Firestore filtered by type (Rent or Resale)
 */
export async function fetchPropertiesFromDB(typeFilter: 'Rent' | 'Resale'): Promise<PropertyCard[]> {
  try {
    ...
    querySnapshot.forEach((doc) => {
      properties.push({ id: doc.id, ...doc.data() } as PropertyCard);
    });

    return properties;
  } catch (error) {
    console.error('Firestore database fetching failed: ', error);
    // Fallback mock data matching New Cairo context if Firestore is unavailable during init
    const fallback: PropertyCard[] = [
      {
        id: 'MVD-V1',
        sbrCode: 'MVD-3F-75K',
    ...
    return fallback.filter((item) => item.type === typeFilter);
  }
}
