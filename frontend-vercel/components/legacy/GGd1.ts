  ...
  try {
    ...
    for (const { rowIndex, row } of pendingRows) {
      ...
      try {
        ...
        // BOT A RULE: write ONLY to scraper_inbox — never to units/listings
        const row = buildScraperInboxRow(rawMessage, from || 'Unknown', groupName || 'Unknown', parsed);
        const docRef = await adminDb.collection(COLLECTIONS.scraperInbox).add(row);

        // Mark sheet row as PROCESSED
        await sheets.spreadsheets.values.update({
          spreadsheetId,
          range: `${SHEET_TAB}!F${rowIndex}`,
          valueInputOption: 'USER_ENTERED',
        ...
        results.processed++;
        console.log(`[CRON:ingest-from-sheets] ✅ Row ${rowIndex} → scraper_inbox/${docRef.id} (status: ${row.status})`);
      } catch (rowErr: any) {
    ...
    // Log summary to Firestore activities
    if (results.processed > 0) {
      await adminDb.collection(COLLECTIONS.activities).add({
        type: 'scraper_inbox_ingest',
        actorId: 'system',
        actorName: 'Scraper Bot A',
        description: `Daily scrape: **${results.processed} rows** → scraper_inbox, **${results.failed}** failed.`,
        text: `Daily scrape: **${results.processed} rows** → scraper_inbox, **${results.failed}** failed.`,
