...
/**
 * Sierra AI WHATSAPP INTELLIGENCE SERVICE
...
export class WhatsAppParserService {
  ...
  static async parseMessage(content: string, media?: { data: string, mimeType: string }) {
    ...
    const modelName = media ? "gemini-pro-latest" : "gemini-flash-latest";
    const model = genAI.getGenerativeModel({ model: modelName });

    const systemInstruction = `ROLE: You are the Sierra AI Strategic Intelligence Parser (The Scribe).
