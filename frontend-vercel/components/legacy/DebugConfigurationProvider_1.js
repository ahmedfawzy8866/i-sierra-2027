...
exports.BrightScriptDebugConfigurationProvider = void 0;
...
const roku_deploy_2 = require("roku-deploy");
class BrightScriptDebugConfigurationProvider {
    constructor(context, telemetryManager, extensionOutputChannel, userInputManager, brightScriptCommands) {
        this.context = context;
        this.telemetryManager = telemetryManager;
        this.extensionOutputChannel = extensionOutputChannel;
        ...
        this.configDefaults = {
            ...
            files: [...roku_deploy_1.DefaultFiles],
    ...
    async resolveDebugConfiguration(folder, config, token) {
        ...
        try {
            ...
            (_a = result.stagingDir) !== null && _a !== void 0 ? _a : (result.stagingDir = path.join('${outDir}/.roku-deploy-staging'));
            ...
            statusbarItem.show();
            try {
                deviceInfo = await roku_deploy_2.rokuDeploy.getDeviceInfo({ host: result.host, remotePort: result.remotePort, enhance: true, timeout: 4000 });
            }
            catch (e) {
            ...
            this.extensionOutputChannel.append(e.stack);
            throw e;
        }
        finally {
            ...
            (_b = this.telemetryManager) === null || _b === void 0 ? void 0 : _b.sendStartDebugSessionEvent(this.processUserWorkspaceSettings(config), result, deviceInfo);
        }
    }
    /**
     * There are several debug-level config values that can be stored in user settings, so get those
     */
    processUserWorkspaceSettings(config) {
        const workspaceConfig = util_1.util.getConfiguration('brightscript.debug');
        let userWorkspaceSettings = {};
        ...
        const result = {
            ...userWorkspaceSettings !== null && userWorkspaceSettings !== void 0 ? userWorkspaceSettings : {},
            ...cloneDeep(config !== null && config !== void 0 ? config : {})
        };
        return result;
    }
    /**
     ...
     */
    async sanitizeConfiguration(config, folder) {
        var _a, _b, _c, _d, _e, _f, _g, _h, _j;
        let userWorkspaceSettings = util_1.util.getConfiguration('brightscript') || {};
        ...
        else if (((_a = vscode.workspace.workspaceFolders) === null || _a === void 0 ? void 0 : _a.length) === 1) {
        ...
        if (config.componentLibraries) {
            ...
            for (let library of config.componentLibraries) {
                ...
                library.files = library.files ? library.files : [...roku_deploy_1.DefaultFiles];
            }
        }
        else {
        ...
        for (const key in this.configDefaults) {
            (_b = config[key]) !== null && _b !== void 0 ? _b : (config[key] = this.configDefaults[key]);
        }
        ...
        if (typeof config.remoteControlMode === 'boolean') {
            ...
            config.remoteControlMode = {
                activateOnSessionStart: (_d = (_c = config.remoteControlMode) === null || _c === void 0 ? void 0 : _c.activateOnSessionStart) !== null && _d !== void 0 ? _d : this.configDefaults.remoteControlMode.activateOnSessionStart,
                deactivateOnSessionEnd: (_f = (_e = config.remoteControlMode) === null || _e === void 0 ? void 0 : _e.deactivateOnSessionEnd) !== null && _f !== void 0 ? _f : this.configDefaults.remoteControlMode.deactivateOnSessionEnd
            };
        }
        if (config.request !== 'launch') {
            await vscode.window.showErrorMessage(`roku-debug only supports the 'launch' request type`);
        }
        if ((_g = config.raleTrackerTaskFileLocation) === null || _g === void 0 ? void 0 : _g.includes('${workspaceFolder}')) {
        ...
        if ((_h = config.packagePath) === null || _h === void 0 ? void 0 : _h.includes('${workspaceFolder}')) {
            config.packagePath = path.normalize(config.packagePath.replace('${workspaceFolder}', folderUri.fsPath));
        }
        if ((_j = config.packagePath) === null || _j === void 0 ? void 0 : _j.includes('${outDir}')) {
    ...
    async processLogfilePath(folder, config) {
        var _a;
        if ((_a = config === null || config === void 0 ? void 0 : config.logfilePath) === null || _a === void 0 ? void 0 : _a.trim()) {
            ...
            try {
                config.logfilePath = roku_debug_1.fileUtils.standardizePath(config.logfilePath);
    ...
    processDapLogFilePath(folder, config) {
        var _a;
        if (!config.debugAdapterProtocolLogging) {
            return config;
        }
        const folderPath = (_a = folder === null || folder === void 0 ? void 0 : folder.uri.fsPath) !== null && _a !== void 0 ? _a : process.cwd();
     ...
     * @param config current config object
     */
    async processEnvFile(folder, config) {
     ...
     * @param config  current config object
     */
    async processHostParameter(config) {
        var _a;
        if (config.host.trim() === '${promptForHost}' || ((_a = config === null || config === void 0 ? void 0 : config.deepLinkUrl) === null || _a === void 0 ? void 0 : _a.includes('${promptForHost}'))) {
     ...
     * @param config  current config object
     */
    async processPasswordParameter(config) {
     ...
     * @param config  current config object
     */
    async processDeepLinkUrlParameter(config) {
     ...
     */
    async openInputBox(placeHolder, value = '') {
     ...
     */
    getBsConfig(workspaceFolder) {
        //try to load bsconfig settings
        let settings = util_1.util.getConfiguration('brightscript', workspaceFolder);
        let configFilePath = settings.get('configFile');
        ...
        let workspaceFolderPath = brighterscript_1.util.uriToPath(workspaceFolder.toString());
        configFilePath = path.resolve(workspaceFolderPath, configFilePath);
        try {
            let bsconfig = brighterscript_1.util.loadConfigFile(configFilePath);
            return bsconfig;
        }
        catch (e) {
...
exports.BrightScriptDebugConfigurationProvider = BrightScriptDebugConfigurationProvider;
