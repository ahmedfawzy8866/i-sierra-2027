    ...
    <div className={`${container} space-y-10`}>
        ...
        <div className="relative z-20 flex min-h-[86vh] flex-col justify-between p-6 md:p-10 xl:p-14">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <span className="hidden rounded-full px-4 py-2 text-xs uppercase tracking-[0.28em] md:inline-block"
              style={{ background: `rgba(15,23,42,0.55)`, border: `1px solid rgba(226,232,240,0.12)`, color: C.muted }}>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {matchedProperties.map((item, i) => (
                <motion.div
                  ...
                  style={{ background: `rgba(241,245,249,0.04)`, border: `1px solid rgba(226,232,240,0.10)` }}
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        ...
        <ValueCard icon={Crown} title="50% VIP Discount"
          description="A dedicated luxury entry offer for non-Egyptians, paired with direct WhatsApp advisory." silver />
      </section>
...
function ValueCard({ icon: Icon, title, description, featured = false, silver = false }) {
  return (
    <div className={`rounded-[28px] p-6 md:p-7 ${glass}`}
      style={silver ? { border: `1px solid rgba(226,232,240,0.30)`, background: `rgba(226,232,240,0.05)` } : {}}>
      <div className="space-y-4">
        ...
        <h3 className="font-[Cormorant_Garamond] text-3xl leading-none" style={{ color: silver ? C.silverLight : C.white }}>{title}</h3>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className={`rounded-[32px] p-4 md:p-5 ${glass}`}>
          ...
          <div className="relative min-h-[620px] overflow-hidden rounded-[28px]"
            style={{ background: `linear-gradient(180deg, rgba(15,23,42,0.60), ${C.navyDeep})`, border: `1px solid rgba(226,232,240,0.10)` }}>
            ...
            <div className="absolute bottom-4 left-4 right-4 flex flex-wrap gap-2">
              {[
                ...
                { label: "Hot Deal",  color: "#94A3B8"     },
              ].map((item) => (
                <div key={item.label} className="rounded-full px-3 py-2 text-xs backdrop-blur-xl"
                  style={{ border: `1px solid rgba(226,232,240,0.15)`, background: `rgba(2,6,23,0.75)`, color: C.offWhite }}>
...
function MapRoads() {
  return (
    <svg className="absolute inset-0 h-full w-full opacity-50" viewBox="0 0 1000 700" fill="none">
      <path d="M40 600C160 520 220 470 340 455C440 440 530 470 660 410C760 365 860 250 960 205"  stroke="rgba(226,232,240,0.15)" strokeWidth="4" />
      <path d="M80 180C220 210 310 250 430 310C520 355 620 360 800 340"                          stroke="rgba(226,232,240,0.12)" strokeWidth="3" />
      <path d="M180 70C230 160 270 250 230 360C190 470 120 540 70 650"                           stroke="rgba(226,232,240,0.10)" strokeWidth="3" />
      <path d="M640 60C610 140 560 240 575 355C590 480 700 590 820 660"                          stroke="rgba(226,232,240,0.12)" strokeWidth="3" />
      <path d="M290 120C385 145 505 185 580 245C675 320 705 405 770 520"                         stroke="rgba(226,232,240,0.08)" strokeWidth="2.5" />
      <path d="M350 610C470 570 530 520 610 520C690 520 770 555 920 610"                         stroke="rgba(226,232,240,0.14)" strokeWidth="3.5" />
    </svg>
  );
}
    ...
    <div className="absolute -translate-x-1/2 -translate-y-1/2" style={{ top, left }}>
      <div className="flex flex-col items-center gap-2">
        ...
        <div className="relative flex h-6 w-6 items-center justify-center rounded-full"
          style={{ background: color, border: `2px solid rgba(226,232,240,0.4)`, boxShadow: `0 0 20px ${color}` }}>
          <MapPinned className="h-3.5 w-3.5" style={{ color: C.navyDeep }} />
        </div>
        <div className="rounded-full px-3 py-1 text-xs backdrop-blur-xl"
          style={{ border: `1px solid rgba(226,232,240,0.22)`, background: `rgba(6,14,26,0.80)`, color: C.offWhite }}>
          {label}
        </div>
      </div>
    </div>
...
function ScoreCard({ label, value, tone }) {
  const tones = {
    rose:  { bg: "rgba(244,63,94,0.12)",  border: "rgba(244,63,94,0.22)",  text: "#fda4af" },
    amber: { bg: "rgba(118, 141, 171, 0.13)", border: "rgba(245,158,11,0.22)", text: "#fcd34d" },
    slate: { bg: "rgba(100,116,139,0.12)",border: "rgba(100,116,139,0.22)",text: "#94a3b8" },
  };
  const t = tones[tone];
  return (
    <div className="rounded-[22px] p-5"
