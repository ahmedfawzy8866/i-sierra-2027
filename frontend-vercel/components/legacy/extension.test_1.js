...
suite('AutoBoot Extension Test Suite', () => {
    ...
    suite('CommandManager', () => {
        let commandManager;
        let mockAutoBootManager;
        let mockStatusBarManager;
        setup(() => {
            mockAutoBootManager = sandbox.createStubInstance(autoBootManager_1.AutoBootManager);
            mockStatusBarManager = sandbox.createStubInstance(statusBarManager_1.StatusBarManager);
            commandManager = new commandManager_1.CommandManager(mockAutoBootManager, mockStatusBarManager);
        });
        ...
        test('should create project with framework selection', async () => {
            sandbox.stub(vscode.window, 'showQuickPick').resolves({ label: 'Next.js', description: 'React framework' });
    ...
    suite('Integration Tests', () => {
        test('should handle workspace folder changes', async () => {
            const autoBootManager = new autoBootManager_1.AutoBootManager();
        ...
        test('should update status bar when project is detected', async () => {
            ...
            autoBootManager.onProjectDetected((projectInfo) => {
                statusBarManager.updateServerStatus(projectInfo.isRunning, projectInfo.framework, projectInfo.port);
            });
