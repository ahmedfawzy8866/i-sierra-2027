/**
 * Sierra AI — WEALTH INTELLIGENCE ENGINE
...
export async function analyzeAssetFinancials(unit: Unit): Promise<FinancialAnalysis> {
  ...
  // --- Sierra AI UPGRADE: Use FinancialService for Valuation Context ---
...
async function generateValuationReasoning(unit: Unit, roi: number, yieldPercent: number): Promise<string> {
  const systemPrompt = `You are the Sierra AI Wealth Analytics Bot (Neural ROI Core V10.0).
