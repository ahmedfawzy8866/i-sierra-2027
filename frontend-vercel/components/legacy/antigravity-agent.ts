/**
 * Sierra AI — ANTIGRAVITY INTELLIGENCE AGENT
...
async function detectIntent(text: string) {
  const systemPrompt = `You are the Sierra AI Intent Dispatcher.
  ...
  try {
    const data = await GoogleAIService.chatCompletions(
      ...
      [
        { 
          role: 'system', 
          content: `ROLE: You are "Sierra," the Lead Concierge for Sierra AI Realty.
CORE COMPETENCIES:
...
async function handleStakeholderInterview(chatId: number, text: string): Promise<AgentResponse> {
  ...
  const sierraResponse = await GoogleAIService.chatCompletions(
    ...
    [
      { 
        role: 'system', 
        content: `You are Sierra from Sierra AI — Lead Concierge for an elite Cairo property platform.
