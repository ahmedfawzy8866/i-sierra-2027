...
exports.BrightScriptPseudoterminal = exports.BrightScriptTaskProvider = void 0;
const vscode = require("vscode");
const path = require("path");
const childProcess = require("child_process");
const os = require("os");
const util_1 = require("./util");
class BrightScriptTaskProvider {
    constructor() {
    ...
    resolveTask(task) {
        ...
        const execution = new vscode.CustomExecution(() => {
            var _a;
            return Promise.resolve(new BrightScriptPseudoterminal(command, definition, (_a = task.scope) !== null && _a !== void 0 ? _a : vscode.TaskScope.Workspace));
        });
        const result = new vscode.Task(definition, (_a = task.scope) !== null && _a !== void 0 ? _a : vscode.TaskScope.Workspace, task.name, 'brightscript', execution);
...
exports.BrightScriptTaskProvider = BrightScriptTaskProvider;
class BrightScriptPseudoterminal {
    constructor(command, taskDefinition, taskScope) {
    ...
    async open() {
        ...
        try {
            ...
            const shell = ((_a = taskOptions.shell) === null || _a === void 0 ? void 0 : _a.executable) || shellConfig.shell;
            const shellArgs = ((_b = taskOptions.shell) === null || _b === void 0 ? void 0 : _b.args) || shellConfig.shellArgs;
            ...
            const cwd = taskOptions.cwd || (workspaceFolder === null || workspaceFolder === void 0 ? void 0 : workspaceFolder.uri.fsPath);
            ...
            this.currentProcess = childProcess.spawn(shell, [...shellArgs, resolvedCommand], {
                env: mergedEnv,
                cwd: cwd
            });
            (_c = this.currentProcess.stdout) === null || _c === void 0 ? void 0 : _c.on('data', (data) => {
                // Pass through output with normalized line endings for VS Code
                this.write(data.toString());
            });
            (_d = this.currentProcess.stderr) === null || _d === void 0 ? void 0 : _d.on('data', (data) => {
                // Pass through output with normalized line endings for VS Code
                this.write(data.toString());
            });
            this.currentProcess.on('exit', (code) => {
                this.exit(code !== null && code !== void 0 ? code : 0);
            });
    ...
    getShellConfiguration() {
        const config = util_1.util.getConfiguration('terminal.integrated');
        let platformKey;
        ...
        const env = config.get(`env.${platformKey}`) || {};
        // 1. Automation profile — VS Code checks this first for tasks/automation
        const automationProfile = config.get(`automationProfile.${platformKey}`);
        if (automationProfile === null || automationProfile === void 0 ? void 0 : automationProfile.path) {
            return {
        ...
        const defaultProfileName = config.get(`defaultProfile.${platformKey}`);
        const profiles = config.get(`profiles.${platformKey}`) || {};
        const defaultProfile = defaultProfileName ? profiles[defaultProfileName] : undefined;
        if (defaultProfile === null || defaultProfile === void 0 ? void 0 : defaultProfile.path) {
     ...
     */
    buildShellArgs(shellPath, profileArgs) {
        var _a;
        if (process.platform !== 'win32') {
        ...
        const basename = ((_a = shellPath.split(/[/\\]/).pop()) !== null && _a !== void 0 ? _a : '').toLowerCase();
     ...
     */
    async getWorkspaceFolderFromScope(taskScope) {
    ...
    async resolveCommandVariables(command, workspaceFolder) {
     ...
     */
    resolveWorkspaceVariables(command, workspaceFolder) {
        let resolvedCommand = command;
     ...
     */
    resolveFileVariables(command, workspaceFolder) {
        let resolvedCommand = command;
     ...
     */
    resolveEditorVariables(command, workspaceFolder) {
        let resolvedCommand = command;
     ...
     */
    resolveSystemVariables(command, workspaceFolder) {
        let resolvedCommand = command;
     ...
     */
    async resolveFolderForFileVariable(command, workspaceFolder) {
        ...
        for (const [globPattern, fullMatches] of uniquePatterns) {
            ...
            const folderPaths = Array.from(folders.keys()).sort();
...
exports.BrightScriptPseudoterminal = BrightScriptPseudoterminal;
//# sourceMappingURL=BrightScriptTaskProvider.js.map