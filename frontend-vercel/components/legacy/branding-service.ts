  ...
  static async brandPropertyImage(
    ...
    try {
      ...
      if (fs.existsSync(this.LOGO_PATH)) {
        ...
        .composite([{
          input: Buffer.from(`<svg><text x="10" y="40" font-family="Arial" font-size="24" fill="white">Sierra AI</text></svg>`),
          top: 0,
          left: 0
        }])
        .png()
            ...
            input: Buffer.from(
              `<svg width="400" height="50">
                <text x="10" y="40" font-family="Inter, sans-serif" font-size="20" fill="white" fill-opacity="0.5">
                  REF: ${propertyCode} | Sierra AI REALTY
                </text>
              </svg>`
            ),
            gravity: 'southwest',
