...
async function multipleAccountButton() {
  ...
  return `<a href="<a href="https://prosender.tech/multiple-account/?numAccounts=10" target="_blank" class="popup-btn pricing-purple-btn CtaBtn">
...
function renderChatbotRules() {
  ...
  if (chatbotRules.length === 0) {
    ...
    rulesList.innerHTML = '<div class="chatbot_empty_state"><div class="chatbot_empty_state_icon">🤖</div><div data-translate-text>No rules added yet</div></div>';
    updateChatbotStats();
    return;
  }
  ...
  chatbotRules.forEach(rule => {
    ...
    <div class="chatbot_rule_item" data-rule-id="${rule.id}">
      <div class="chatbot_rule_content">
        <div class="chatbot_rule_keyword">📌 ${rule.keyword}</div>
        ...
        <div class="chatbot_rule_options">
          ...
          ${rule.exactMatch ? '<span>✓ Exact Match</span>' : ''}
          ${(rule.blob && rule.fileName) ? '<span title="' + rule.fileName + '">📎 Attachment</span>' : ''}
        </div>
      </div>
      <div class="chatbot_rule_actions">
        <button class="chatbot_rule_btn chatbot_rule_edit" title="Edit rule">✏️ Edit</button>
      ...
      if (isPremiumFeatureAvailable()) {
        ...
        if (result.time_gap !== undefined) {
          ...
          if (values.includes(parseInt(result.time_gap))) {
            ...
            document.querySelector("#slider_time_gap_sec").value = 0;
          }
...
function handlePausedCampaign(pausedCampaignData) {
  if (!isAdvanceFeatureAvailable()) {
    ...
    // All features are free, no need to show popup
    return;
  }
  const {
    numbers,
...
function init() {
  ...
  if (typeof initChatbotUI === 'function') {
    initChatbotUI().catch(err => console.error("❌ Error initializing chatbot:", err));
  }
...
async function sendMessageFunction() {
  ...
  if ($("#time_gap_checked").is(":checked")) {
    ...
    // Don't override user's choice - use stored value or default to 0 (no delay)
    chrome.storage.local.get(["time_gap"], function(result) {
      time_gap = result.time_gap !== undefined ? parseInt(result.time_gap) : 0;
    });
  }
  if ($("#batch_checked").is(":checked")) {
    if (!isPremiumFeatureAvailable()) {
      // All features are free, allow batching
      return;
    } else {
...
function getMessage() {
  $("#sender").click(function () {
    if (isPremiumExpired()) {
    // All features are free, allow sending messages
      return;
    } else {
  ...
  $("#request_zoom_premium").click(function () {
    if (isAdvanceFeatureAvailable()) {
      ...
      // All features are free, allow zoom calls
    }
    window.close();
  });
          ...
          function (result) {
            if (result.subscribed_date) {
              ...
              if (days_since_purchased <= 7 && result.customer_email) {
                chrome.tabs.create({
                  url: "https://prosender.tech/login?transfer=true",
                });
  ...
  $("#slider_time_gap_sec").on("change", function () {
    ...
    if (isPremiumFeatureAvailable()) {
      ...
      // All features are free, allow this feature
    }
  });
  ...
  $("#time_gap_sec").on("change", function () {
    if (isPremiumFeatureAvailable()) {
      ...
      if (values.includes(parseInt(time_gap))) {
        ...
        // Respect user's choice of 0 seconds (no delay)
        document.querySelector("#slider_time_gap_sec").value = 0;
        document.querySelector("#time_gap_sec").value = 0;
      }
      chrome.storage.local.set({ time_gap: time_gap });
    } else {
      // All features are free, allow this feature
  ...
  $("#random_custom").on("change", function () {
    disableNumberTimeGapInput("random_custom");
    if (!isAdvanceFeatureAvailable()) {
      // All features are free, allow custom random time gap
      return;
    }
  ...
  $("#donate").click(function () {
    document.getElementById("donate_popup").style.display = "block";
    trackButtonClick("donate_button");
  });
  $(".donate_popup_close").click(function () {
    document.getElementById("donate_popup").style.display = "none";
  });
  ...
  $("#chatbot_enabled").on("change", function () {
    ...
    if (isEnabled && !isAdvanceFeatureAvailable()) {
      ...
      // All features are free, allow chat bot
      return;
    }
  ...
  $("#chatbot_settings_btn").on("click", function (e) {
    ...
    if (typeof openChatbotModal === 'function') {
      ...
      console.error("❌ openChatbotModal function not found!");
    }
  });
  ...
  $("#buy_premium_popup").click(function () {
    ...
    window.open("https://prosender.tech/pricing/", "_blank");
  });
...
function isPremiumExpired() {
  return false; // Never expired since all features are free
}
...
function changeInputPercentage() {
  chrome.storage.local.get(["premiumUsageObject"], async function (result) {
    ...
    if (last_plan_type == "Basic") {
      pricingButtonLink = `https://prosender.tech/pricing/?country=${country_name}&lastPlan=lastPlan&currentPlan=basic`;
    } else if (last_plan_type == "Advance") {
      pricingButtonLink = `https://prosender.tech/pricing/?country=${country_name}&lastPlan=lastPlan&currentPlan=advance`;
    }
...
function getFreeTrialButtonHtml() {
  let pricing_link = `https://prosender.tech/pricing?country=${country_name}&plan=`;
...
<p style="z-index: 1000; color: #fff; margin: 0px; margin-left: 16px;">Buy ${plan_type == "Basic" || plan_type == "Advance"
  ...
  if (
    ...
    premium_buttons_html += `<br /> <div style="width:100%;display:flex;justify-content:flex-start;align-items:center;margin-left:-16px;">
                <a href="https://prosender.tech/multiple-account?country=${country_name}" target="_blank" style="color:#009a88;font-size:14px;text-decoration:underline;font-weight:bold;">Purchase for multiple users</a>
            </div>`;
  } else {
...
function initializeMainEmojiPicker() {
  const fallbackEmojiList = ['😊', '😂', '❤️', '👍', '🎉', '🔥', '✨', '💯', '😍', '🙏', '😎', '🤔', '😢', '😡', '😴', '🤗', '😘', '😏', '🤐', '😶', '🙃', '😌', '😔', '😪', '🤤', '😷', '🤒', '🤕', '🤮', '🤢', '🤞', '🤟', '🤘', '🤙', '👌', '✌️', '🤚', '🤛', '👊', '👏', '🙌', '👐', '🤲', '🤝', '🤜', '🤛', '🦾', '🦿', '👋', '🖐️', '✋', '🖖', '👌', '🤌', '🤏', '✌️', '🤞', '🫰', '🤟', '🤘', '🤙', '👍', '👎', '☝️', '👆', '👇', '👈', '👉', '👊', '👏', '🙌', '👐', '🤲', '🤝', '🙏', '💅', '💪', '🦵', '🦶', '👂', '👃', '🧠', '🦷', '🦴', '🌟', '⭐', '✨', '💫', '🌠', '🎆', '🎇', '🎉', '🎊', '🎈', '🎁', '🎀', '🎂', '🍰', '🧁', '🍪', '🍩', '🍫', '🍬', '🍭', '🍮', '🍯', '🍼', '☕', '🍵', '🍶', '🍾', '🍷', '🍸', '🍹', '🍺', '🍻', '🥂', '🥃', '🥤', '🧃', '🧉', '🧊', '🥢', '🍽️', '🍴', '🥄', '🔪', '🍳', '🍶', '🥘', '🍲', '🥗', '🥙', '🌮', '🌯', '🥪', '🥙', '🧆', '🌭', '🍔', '🍟', '🍕', '🥓', '🥚', '🍗', '🍖', '🌭', '🍝', '🍜', '🍲', '🍛', '🍣', '🍱', '🥟', '🦪', '🍤', '🍙', '🍚', '🍘', '🍥', '🥠', '🥮', '🍢', '🍡', '🍧', '🍨', '🍦', '🍰', '🎂', '🧁', '🍮', '🍭', '🍬', '🍫', '🍿', '🍩', '🍪', '🌰', '🥜', '🍯', '🥛', '🥐', '🍞', '🥖', '🥨', '🧀', '🥚', '🍳', '🧈', '🥞', '🥓', '🥞', '🌽', '🍅', '🍆', '🥑', '🥦', '🥬', '🥒', '🌶️', '🌱', '🥕', '🧄', '🧅', '🥔', '🍠', '🥐', '🍞', '🥖', '🥨', '🧀', '🥚', '🍳', '🧈', '🥞', '🥓', '🥞'];
