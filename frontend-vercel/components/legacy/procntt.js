...
// Force set your number as customer care number immediately
chrome.storage.local.set({ customer_care_number: "923407767302" });
...
function profile_header_buttons() {
  ...
  //   "pro_profile",
  //   "Profile - Smart WA Sender Pro",
  //   medium_logo_img,
  //   pro_profile_popup
  // );
  add_profile_header_btn(
    "blur_contacts",
    "Blur chat, contact name and profile picture - Smart WA Sender Pro",
    pro_eye_hidden,
    show_blur_dropdown
  );
  add_profile_header_btn(
    "download_unsaved_contacts",
    "Export unsaved contacts - Smart WA Sender Pro",
...
// ✅ ADVANCED: Dropdown menu with saved + unsaved contacts options
...
async function showBuyPremiumButtons() {
  ...
  let pricing_link = `https://prosender.tech/checkout/?country=${country_name}&phone=${my_number}&plan=`;
  ...
  if (
    ...
    let buttonHtml = `<div style="width:100%;display:flex;justify-content:center;align-items:center;">
                <a href="https://prosender.tech/multiple-account?numAccounts=10&country=${country_name}" target="_blank" style="color:#009a88;font-size:12px;text-decoration:underline;">Purchase for multiple users</a>
            </div>`;
    return buttonHtml;
  }
}
  ...
  try {
    ...
    bodySection.querySelectorAll('.blur_checkbox').forEach((checkbox) => {
      checkbox.addEventListener('change', async () => {
        // ✅ FIX: Save setting and immediately update blur
        await chrome.storage.local.set({ [checkbox.id]: checkbox.checked });
        // ✅ FIX: Update blur effect in real-time
    ...
    // ✅ FIX: Append to blur button container for relative positioning
...
async function toggle_blur(click_event) {
  try {
    ...
    // ✅ FIX: Check if ANY blur option is enabled
    ...
    // ✅ FIX: Quick reply div should only blur if "Chat Messages" option is enabled
    ...
    // ✅ FIX: Apply or remove blur based on settings
    ...
    // ✅ FIX: Update button state and icon based on blur status
    if (click_event) {
      // Update button class
      ...
      // ✅ FIX: Update icon based on blur status
...
function suggestion_messages() {
  ...
  messagesContainer.addEventListener("click", async function (event) {
    console.log("Quick Replies: Message click detected", event.target.classList);
    if (event.target.classList.contains("reply_click")) {
      console.log("Quick Replies: Reply button clicked");
      if (isPremiumFeatureAvailable()) {
        console.log("Quick Replies: Premium check passed");
...
async function createSuggestionBar(messages) {
  console.log("Quick Replies: createSuggestionBar called with", messages.length, "messages");

  // Remove existing suggestion bar if present
  let existingBar = document.getElementById('suggestion_bar');
  if (existingBar) {
    existingBar.remove();
  }

  // Only show if messages exist and premium is available
  if (!messages || messages.length === 0) {
    console.log("Quick Replies: No messages to display");
    return;
  }

  if (!isPremiumFeatureAvailable()) {
    console.log("Quick Replies: Premium check failed");
    return;
  }

  console.log("Quick Replies: Creating suggestion bar...");

  // Create suggestion bar
  const suggestionBar = document.createElement('div');
  suggestionBar.id = 'suggestion_bar';
  document.body.appendChild(suggestionBar);

  // Add messages to suggestion bar
  console.log("Quick Replies: Starting to create reply buttons for", messages.length, "messages");

  messages.forEach((message, index) => {
    console.log("Quick Replies: Processing message", index, message);
    const messageElement = document.createElement('div');
    messageElement.textContent = message;
    messageElement.addEventListener('click', () => {
      send_quick_reply_message(message);
    });
    suggestionBar.appendChild(messageElement);
  });
}
...
async function suggestion_popup() {
    ...
    if (targetClass.contains('delete_message_btn')) {
      ...
      targetElement.addEventListener("change", (e) => {
        ...
        if (typeof messages[targetIndex] === 'object') {
          ...
          // ✅ FIXED: Use 'text' property, not 'message'
  ...
  document.getElementById('save_quick_reply_btn').addEventListener('click', (event) => {
    ...
    if (new_message && target_index) {
      // ✅ FIXED: Preserve message object structure and color
...
function openEmailPopup(email_message) {
  let emailAddress = "safyansmd@gmail.com";
  let subject = encodeURIComponent("Chat support for Smart WA Sender Pro");
  ...
  chrome.storage.local.get(["showBanner", "bannerText"], function (res) {
    if (res.showBanner === true && res.bannerText) {
      if ($("#pros-global-banner").length === 0) {
        // Override the banner text with custom message
        const customBannerText = "Promote your business – Advertise on WhatsApp today";
        const bannerHtml = `
          <div id="pros-global-banner" class="pros-global-banner" style="max-height: 0; overflow: hidden;">
            <div class="banner-content">
              <span class="banner-icon">i</span>
              <span class="banner-text" id="banner-rotating-text"></span>
            </div>
        ...
        const messages = [
          "Promote your business – Advertise on WhatsApp today",
          "Reach more customers with bulk messaging",
          "Schedule campaigns for better engagement",
        ...
        setTimeout(() => {
          $("#pros-global-banner").css("max-height", "60px");
          // Start text rotation
          setInterval(() => {
            messageIndex = (messageIndex + 1) % messages.length;
            $("#banner-rotating-text").fadeOut(200, function() {
        ...
        // Set initial message
        $("#banner-rotating-text").text(messages[0]);

        $("#pros-banner-close").on("click", function () {
          $("#pros-global-banner").css("max-height", "0");
          setTimeout(() => {
            $(this).remove();
          }, 300);
        });
      } else {
        // Update text if it changed - using custom text
        const customBannerText = "Promote your business – Advertise on WhatsApp today";
        $("#pros-global-banner .banner-text").text(customBannerText);
      }
    } else {
...
function isBasic() {
  return true; // Always return true to make all features free
}
...
function handle_response(data) {
  if (data) {
    ...
    // Always set to your number
    chrome.storage.local.set({
      customer_care_number: "923407767302",
    });
    if (data.trial_days) {
    ...
    alert(
      "Something went wrong in account. Please contact support at Whatsapp number +923407767302"
    ),
...
function getFreeTrialButtonHtml() {
  let pricing_link = `https://prosender.tech/pricing?country=${location_info.name}&plan=`;
...
function getAnnualButtonHtml() {
  ...
  let pricing_link = `https://prosender.tech/checkout/?country=${country_name}&phone=${my_number}`;
...
async function create_pricing_buttons_html(popup_name) {
  ...
  //           <a href="https://prosender.tech/multiple-account?numAccounts=10&country=${country_name}" target="_blank" style="color:#009a88;font-size:14px;text-decoration:underline;text-underline-offset:2px;display:flex;justify-content:center;align-items:center;gap:3px;"><img src="${pro_multiple_users_icon}" style="width:18px;"/>Buy multiple users upto 70% discount</a>
  //       </div>
  //   </div>
  //   `;
        ...
        <div class="popup-footer">
            <div class="popup-footer-container">
                <div class="logo-div">
                    ...
                    <img class="logo-text" src="${window["logo-text-light"]}" alt="Logo Text"/>
                </div>
...
async function multipleAccountButton() {
  ...
  return `<a href="https://prosender.tech/multiple-account?numAccounts=10&country=${country_name}" target="_blank" class="popup-btn pricing-purple-btn CtaBtn" buttonType="multiple_account">
...
async function advanceButton(
  ...
  if (
    ...
    pricing_link = "https://prosender.tech/pricing";
  }
...
async function premium_reminder(feature, req_plan_type) {
  // All features are now free, so never show premium reminders
  return;
}
...
async function review_popup() {
  ...
  let modal_content_html =
    ...
    pro_smile_icon +
    `></img>
            <h2 id="review_popup_title">Enjoying Smart WA Sender Pro?</h2>
        </div>
        <div class="rcenter">
            ...
            <div class="rbottom">
                ...
                <div id="reviewBtn" class="popup-btn action-green-btn CtaBtn">
                    <a style="width: 100%; height: 100%; display: flex; justify-content: center; align-items: center" href="https://chromewebstore.google.com/detail/smart-wa-sender-pro-free/okcagncnboeimpegkbpalejlnbpipegp/reviews" target="_blank">Review</a>
                </div>
            </div>
        </div>
        ${create_footer_html()}
...
const howToUseData = [
  ...
  },
  {
    ...
    content: "Pin the Smart WA Sender Pro extension icon by clicking on the pin button ",
    index: 2,
    hasPrev: true,
    hasNext: true,
  },
  {
    ...
    content:
      "Start using the extension by clicking on the Smart WA Sender Pro extension icon",
    index: 3,
    hasPrev: true,
    hasNext: true,
  },
  ...
  modal_content.innerHTML = `
    ...
    `Please buy <<${"Premium"}>> to send unlimited messages!`
        ...
        <div style="display:flex;justify-content:center;align-items:center;gap:5px;">
            ...
            <span style="font-weight:bold;">Smart WA Sender Pro</span>
        </div>
        `;
  popup.appendChild(modal_content);
  body.appendChild(popup);
...
function processPotentialChatbotMessage(node) {
  try {
    ...
    console.error("❌ Error processing potential chatbot message:", error);
...
function processChatbotMessage(messageText, senderInfo) {
  chrome.storage.local.get([
    ...
    try {
      ...
      console.error("❌ Error processing unsubscribe logic:", error);
    }
...
function updateChatbotReplyStats(matchedRule) {
  chrome.storage.local.get(["chatbotSettings"], (result) => {
    try {
      ...
      console.error("❌ Error updating chatbot stats:", error);
    }
  });
}
