...
export class AutoBootManager {
    private projectInfo: ProjectInfo | null = null;
    private onProjectDetectedEmitter = new vscode.EventEmitter<ProjectInfo>();
    private onServerStatusChangedEmitter = new vscode.EventEmitter<boolean>();

    public readonly onProjectDetected = this.onProjectDetectedEmitter.event;
    public readonly onServerStatusChanged = this.onServerStatusChangedEmitter.event;

    async detectProjectType(projectPath: string): Promise<ProjectInfo | null> {
        try {
            // Check license first
            ...
            if (licenseInfo.accessLevel === 'free') {
                vscode.window.showInformationMessage(
                    'AutoBoot: Using free tier (3 languages). Upgrade for full access to 9 languages!',
                    'Upgrade Now'
                ).then(selection => {
            ...
            if (projectInfo) {return this.setProjectInfo(projectInfo);}
    ...
    private async setProjectInfo(projectInfo: any): Promise<any> {
    ...
    private async detectJavaScriptProject(projectPath: string): Promise<any> {
    ...
    private async detectJavaProject(projectPath: string): Promise<any> {
        ...
        if (fs.existsSync(path.join(projectPath, 'pom.xml'))) {
            ...
            const { devScript, startCommand, port } = await this.getSmartJavaCommands(
                framework, 'maven', projectPath, mainClasses, readmeInstructions, hasDockerfile
            );
        ...
        if (fs.existsSync(path.join(projectPath, 'build.gradle')) || fs.existsSync(path.join(projectPath, 'build.gradle.kts'))) {
            ...
            const { devScript, startCommand, port } = await this.getSmartJavaCommands(
                framework, 'gradle', projectPath, mainClasses, readmeInstructions, hasDockerfile
            );
    ...
    private async detectPythonProject(projectPath: string): Promise<any> {
    ...
    private async detectClojureProject(projectPath: string): Promise<any> {
    ...
    private async detectGoProject(projectPath: string): Promise<any> {
        ...
        if (fs.existsSync(path.join(projectPath, 'go.mod'))) {
            ...
            const { devScript, startCommand, port } = await this.getSmartGoCommands(
                framework, projectPath, mainFiles, readmeInstructions, hasDockerfile
            );
        ...
        if (goFiles.length > 0) {
            ...
            const { devScript, startCommand, port } = await this.getSmartGoCommands(
                'Go', projectPath, mainFiles, readmeInstructions, hasDockerfile
            );
    ...
    private async detectCSharpProject(projectPath: string): Promise<any> {
        // Check for .NET project files
        const projectFiles = [
            '*.csproj', '*.vbproj', '*.fsproj', // Individual project files
            '*.sln', // Solution files
        ...
        for (const pattern of projectFiles) {
            ...
            if (files.length > 0) {
                hasProjectFile = true;
                if (files[0].endsWith('.csproj')) {projectType = 'C#';}
                else if (files[0].endsWith('.vbproj')) {projectType = 'VB.NET';}
                else if (files[0].endsWith('.fsproj')) {projectType = 'F#';}
                else if (files[0].endsWith('.sln')) {projectType = 'C# Solution';}
                break;
            }
        }
        ...
        if (hasProjectFile) {
            ...
            const { devScript, startCommand, port } = await this.getSmartDotNetCommands(
                framework, projectPath, mainFiles, readmeInstructions, hasDockerfile
            );
        ...
        const csharpFiles = fs.readdirSync(projectPath).filter(file => 
            file.endsWith('.cs') || file.endsWith('.vb') || file.endsWith('.fs')
        );
        ...
        if (csharpFiles.length > 0) {
            ...
            const { devScript, startCommand, port } = await this.getSmartDotNetCommands(
                '.NET', projectPath, mainFiles, readmeInstructions, hasDockerfile
            );
    ...
    private async detectRustProject(projectPath: string): Promise<any> {
        ...
        if (fs.existsSync(path.join(projectPath, 'Cargo.toml'))) {
            ...
            const { devScript, startCommand, port } = await this.getSmartRustCommands(
                framework, projectPath, mainFiles, readmeInstructions, hasDockerfile
            );
        ...
        if (rustFiles.length > 0) {
            ...
            const { devScript, startCommand, port } = await this.getSmartRustCommands(
                'Rust', projectPath, mainFiles, readmeInstructions, hasDockerfile
            );
    ...
    private async detectRubyProject(projectPath: string): Promise<any> {
        // Check for Ruby project indicators
        const rubyIndicators = [
            'Gemfile',           // Bundler gem file
        ...
        if (hasRubyProject) {
            ...
            const { devScript, startCommand, port } = await this.getSmartRubyCommands(
                framework, projectPath, mainFiles, readmeInstructions, hasDockerfile
            );
        ...
        if (rubyFiles.length > 0) {
            ...
            const { devScript, startCommand, port } = await this.getSmartRubyCommands(
                'Ruby', projectPath, mainFiles, readmeInstructions, hasDockerfile
            );
    ...
    private async detectPHPProject(projectPath: string): Promise<any> {
        ...
        const phpDirectories = [
            'app',           // Laravel, Symfony app directory
        ...
        if (hasPHPProject) {
            ...
            const { devScript, startCommand, port } = await this.getSmartPHPCommands(
                framework, projectPath, mainFiles, readmeInstructions, hasDockerfile
            );
        ...
        if (phpFiles.length > 0) {
            ...
            const { devScript, startCommand, port } = await this.getSmartPHPCommands(
                'PHP', projectPath, mainFiles, readmeInstructions, hasDockerfile
            );
    ...
    private detectJavaFramework(projectPath: string): string {
        try {
            ...
            if (fs.existsSync(pomPath)) {
                ...
                if (pomContent.includes('spring-boot-starter')) {return 'Spring Boot';}
                if (pomContent.includes('helidon-')) {return 'Helidon';}
                if (pomContent.includes('io.helidon')) {return 'Helidon';}
                if (pomContent.includes('quarkus')) {return 'Quarkus';}
                if (pomContent.includes('micronaut')) {return 'Micronaut';}
                if (pomContent.includes('jakarta.ws.rs')) {return 'JAX-RS';}
                if (pomContent.includes('javax.ws.rs')) {return 'JAX-RS';}
            }
            ...
            if (fs.existsSync(gradlePath)) {
                ...
                if (gradleContent.includes('spring-boot')) {return 'Spring Boot';}
                if (gradleContent.includes('helidon')) {return 'Helidon';}
                if (gradleContent.includes('io.helidon')) {return 'Helidon';}
                if (gradleContent.includes('quarkus')) {return 'Quarkus';}
                if (gradleContent.includes('micronaut')) {return 'Micronaut';}
                if (gradleContent.includes('jakarta.ws.rs')) {return 'JAX-RS';}
                if (gradleContent.includes('javax.ws.rs')) {return 'JAX-RS';}
            }
        } catch (error) {
    ...
    private getJavaCommands(framework: string, packageManager: string): { devScript: string, startCommand: string, port: number } {
        switch (framework) {
            case 'Spring Boot':
                return {
    ...
    private detectPythonPackageManager(projectPath: string): string {
        ...
        if (fs.existsSync(path.join(projectPath, 'pyproject.toml'))) {return 'pip';}
        if (fs.existsSync(path.join(projectPath, 'requirements.txt'))) {return 'pip';}
        
        return 'pip';
    }

    private getPythonStartCommand(framework: string, packageManager: string): string {
        switch (framework) {
            case 'Django':
    ...
    private findDevScript(packageJson: any): string {
    ...
    private detectPort(packageJson: any, framework: string): number {
        ...
        // Framework defaults
        const frameworkPorts: { [key: string]: number } = {
            'Next.js': 3000,
            'Vite': 5173,
            'Angular': 4200,
            'Vue.js': 8080,
    ...
    private async analyzeJavaDependencies(projectPath: string): Promise<any> {
        try {
            const results: any = {
                language: 'Java',
    ...
    private async analyzePythonDependencies(projectPath: string): Promise<any> {
        try {
            const results: any = {
                language: 'Python',
                dependencies: [],
    ...
    private async analyzeClojureDependencies(projectPath: string): Promise<any> {
        try {
            const results: any = {
                language: 'Clojure',
                dependencies: [],
    ...
    private async analyzeGoDependencies(projectPath: string): Promise<any> {
        try {
            const results: any = {
                language: 'Go',
                dependencies: [],
    ...
    private async analyzeJavaScriptDependencies(projectPath: string): Promise<any> {
        try {
    ...
    async analyzePerformance(projectPath: string): Promise<any> {
        try {
    ...
    private generatePerformanceRecommendations(buildSize: number, fileCount: number): string[] {
    ...
    private async findMainClass(projectPath: string): Promise<string[]> {
        try {
            const allMainClasses: string[] = [];
    ...
    private async findMavenModules(projectPath: string): Promise<string[]> {
        try {
    ...
    private async searchForAllMainClasses(dir: string, packagePrefix = ''): Promise<string[]> {
        try {
    ...
    private async searchForMainClass(dir: string, packagePrefix = ''): Promise<string | null> {
        try {
            const files = fs.readdirSync(dir);
    ...
    private hasMainMethod(javaContent: string): boolean {
    ...
    private removeJavaComments(javaContent: string): string {
    ...
    private async scanReadmeForInstructions(projectPath: string): Promise<string[]> {
    ...
    private extractRunInstructions(readmeContent: string): string[] {
        const instructions: string[] = [];
    ...
    private hasDockerfile(projectPath: string): boolean {
    ...
    private async getSmartJavaCommands(
        framework: string, 
        packageManager: string, 
        projectPath: string,
        mainClasses: string[],
        readmeInstructions: string[],
        hasDockerfile: boolean
    ): Promise<{ devScript: string, startCommand: string, port: number }> {
    ...
    private findBestReadmeCommand(instructions: string[], packageManager: string): string | null {
        // Prefer commands that match the detected package manager
        const preferredCommands = instructions.filter(cmd => 
            cmd.toLowerCase().includes(packageManager)
        );
        ...
        const javaCommands = instructions.filter(cmd => 
            cmd.toLowerCase().includes('java') || 
    ...
    private getDockerCommand(projectPath: string): string | null {
    ...
    private async detectGoFramework(projectPath: string): Promise<string> {
        try {
            ...
            if (fs.existsSync(goModPath)) {
                ...
                if (goModContent.includes('github.com/gorilla/mux')) {return 'Gorilla Mux';}
                ...
                if (goModContent.includes('github.com/beego/beego')) {return 'Beego';}
                if (goModContent.includes('github.com/revel/revel')) {return 'Revel';}
                if (goModContent.includes('github.com/kataras/iris')) {return 'Iris';}
                if (goModContent.includes('github.com/go-chi/chi')) {return 'Chi';}
                if (goModContent.includes('google.golang.org/grpc')) {return 'gRPC';}
                if (goModContent.includes('github.com/grpc-ecosystem/grpc-gateway')) {return 'gRPC Gateway';}
            }
            ...
            if (fs.existsSync(mainGoPath)) {
                ...
                if (mainContent.includes('gin.Default()') || mainContent.includes('gin.New()')) {return 'Gin';}
                if (mainContent.includes('mux.NewRouter()')) {return 'Gorilla Mux';}
                if (mainContent.includes('echo.New()')) {return 'Echo';}
                if (mainContent.includes('fiber.New()')) {return 'Fiber';}
                if (mainContent.includes('http.ListenAndServe')) {return 'HTTP Server';}
                if (mainContent.includes('grpc.NewServer()')) {return 'gRPC';}
            }
    ...
    private async findGoMainFiles(projectPath: string): Promise<string[]> {
        try {
            const mainFiles: string[] = [];
            
            // Search for main.go files recursively
            const searchForMainGo = (dir: string, relativePath = ''): void => {
                try {
                    const files = fs.readdirSync(dir);
    ...
    private hasGoMainFunction(goContent: string): boolean {
    ...
    private removeGoComments(goContent: string): string {
    ...
    private async getSmartGoCommands(
        framework: string,
        projectPath: string,
        mainFiles: string[],
        readmeInstructions: string[],
    ...
    private getDefaultGoPort(framework: string): number {
        switch (framework) {
            case 'Gin':
            case 'Echo':
            case 'Fiber':
    ...
    private async detectDotNetFramework(projectPath: string): Promise<string> {
        try {
            // Check project files for framework information
            const projectFiles = fs.readdirSync(projectPath).filter(file => 
            ...
            for (const projectFile of projectFiles) {
                ...
                if (projectContent.includes('Microsoft.AspNetCore') || 
                    ...
                    if (projectContent.includes('Microsoft.AspNetCore.Blazor')) {return 'Blazor Server';}
                    if (projectContent.includes('Microsoft.AspNetCore.Components.WebAssembly')) {return 'Blazor WebAssembly';}
                    if (projectContent.includes('Microsoft.AspNetCore.Mvc')) {return 'ASP.NET Core MVC';}
                    if (projectContent.includes('Microsoft.AspNetCore.ApiExplorer')) {return 'ASP.NET Core Web API';}
                    return 'ASP.NET Core';
                }

                // Other frameworks
                if (projectContent.includes('Microsoft.EntityFrameworkCore')) {return 'Entity Framework Core';}
                if (projectContent.includes('Microsoft.Extensions.Hosting')) {return '.NET Generic Host';}
                if (projectContent.includes('Microsoft.AspNetCore.SignalR')) {return 'SignalR';}
                if (projectContent.includes('Microsoft.Extensions.DependencyInjection')) {return '.NET Core';}
                if (projectContent.includes('Xamarin')) {return 'Xamarin';}
                if (projectContent.includes('Microsoft.WindowsDesktop.App')) {return 'WPF/WinForms';}
                if (projectContent.includes('Microsoft.AspNetCore.Grpc')) {return 'gRPC';}
                if (projectContent.includes('Microsoft.Azure.Functions')) {return 'Azure Functions';}

                // Target framework detection
                if (projectContent.includes('<TargetFramework>net8.0</TargetFramework>')) {return '.NET 8';}
                if (projectContent.includes('<TargetFramework>net7.0</TargetFramework>')) {return '.NET 7';}
                if (projectContent.includes('<TargetFramework>net6.0</TargetFramework>')) {return '.NET 6';}
                if (projectContent.includes('<TargetFramework>net5.0</TargetFramework>')) {return '.NET 5';}
                if (projectContent.includes('<TargetFramework>netcoreapp')) {return '.NET Core';}
                if (projectContent.includes('<TargetFramework>netstandard')) {return '.NET Standard';}
                if (projectContent.includes('<TargetFramework>net4')) {return '.NET Framework';}
            }
    ...
    private async findCSharpMainFiles(projectPath: string): Promise<string[]> {
        try {
            const mainFiles: string[] = [];
            
            // Search for Program.cs and Main methods recursively
            const searchForMainFiles = (dir: string, relativePath = ''): void => {
                try {
                    const files = fs.readdirSync(dir);
    ...
    private hasCSharpMainMethod(csharpContent: string): boolean {
        // Remove comments first
    ...
    private removeCSharpComments(csharpContent: string): string {
        // Remove single-line comments (//)
    ...
    private async getSmartDotNetCommands(
        framework: string,
        projectPath: string,
        mainFiles: string[],
        readmeInstructions: string[],
    ...
    private getDefaultDotNetPort(framework: string): number {
        switch (framework) {
            case 'ASP.NET Core':
    ...
    private async analyzeDotNetDependencies(projectPath: string): Promise<any> {
        try {
            const results: any = {
                language: 'C#/.NET',
                dependencies: [],
            ...
            if (projectFiles.length > 0) {
                ...
                try {
                    ...
                    const packageLines = stdout.split('\n').filter(line => 
                        line.includes('>') && !line.includes('Project') && !line.includes('---')
                    );
                ...
                try {
                    ...
                    const vulnLines = vulnOutput.split('\n').filter(line => 
                        line.includes('Vulnerable') || line.includes('Critical') || line.includes('High')
                    );
    ...
    private async detectRustFramework(projectPath: string): Promise<string> {
        try {
            ...
            if (fs.existsSync(cargoTomlPath)) {
                ...
                if (cargoContent.includes('tide =')) {return 'Tide';}
                if (cargoContent.includes('hyper =')) {return 'Hyper';}
                if (cargoContent.includes('tokio-tungstenite =')) {return 'WebSocket (Tokio)';}
                if (cargoContent.includes('tonic =')) {return 'gRPC (Tonic)';}
                
                // Async runtimes
                if (cargoContent.includes('tokio =')) {return 'Tokio';}
                if (cargoContent.includes('async-std =')) {return 'Async-std';}
                
                // CLI frameworks
                if (cargoContent.includes('clap =')) {return 'CLI (Clap)';}
                if (cargoContent.includes('structopt =')) {return 'CLI (StructOpt)';}
                
                // Game engines
                if (cargoContent.includes('bevy =')) {return 'Bevy Game Engine';}
                if (cargoContent.includes('ggez =')) {return 'GGEZ Game Framework';}
                
                // Desktop GUI
                if (cargoContent.includes('tauri =')) {return 'Tauri';}
                if (cargoContent.includes('egui =')) {return 'egui';}
                if (cargoContent.includes('iced =')) {return 'Iced';}
            ...
            if (fs.existsSync(mainRsPath)) {
                ...
                if (mainContent.includes('axum::')) {return 'Axum';}
                if (mainContent.includes('actix_web::')) {return 'Actix Web';}
                if (mainContent.includes('warp::')) {return 'Warp';}
                if (mainContent.includes('rocket::')) {return 'Rocket';}
                if (mainContent.includes('tide::')) {return 'Tide';}
                if (mainContent.includes('hyper::')) {return 'Hyper';}
                if (mainContent.includes('tokio::main')) {return 'Tokio';}
                if (mainContent.includes('async_std::main')) {return 'Async-std';}
    ...
    private async findRustMainFiles(projectPath: string): Promise<string[]> {
        try {
            const mainFiles: string[] = [];
            
            // Search for main.rs and lib.rs files
            const searchForRustFiles = (dir: string, relativePath = ''): void => {
                try {
                    const files = fs.readdirSync(dir);
    ...
    private hasRustMainFunction(rustContent: string): boolean {
        // Remove comments first
    ...
    private removeRustComments(rustContent: string): string {
        // Remove single-line comments (//)
    ...
    private async getSmartRustCommands(
        framework: string,
        projectPath: string,
        mainFiles: string[],
        readmeInstructions: string[],
    ...
    private getDefaultRustPort(framework: string): number {
        switch (framework) {
            case 'Axum':
            case 'Actix Web':
            case 'Warp':
    ...
    private async analyzeRustDependencies(projectPath: string): Promise<any> {
        try {
            const results: any = {
                language: 'Rust',
                dependencies: [],
            ...
            if (fs.existsSync(path.join(projectPath, 'Cargo.toml'))) {
                ...
                try {
                    ...
                    const outdatedLines = outdatedOutput.split('\n').filter(line => 
                        line.includes('->') && !line.includes('Name')
                    );
                    results.outdated = outdatedLines.length;
                    results.outdatedDependencies = outdatedLines;
                } catch (outdatedError) {
    ...
    private async detectRubyFramework(projectPath: string): Promise<string> {
        try {
            // Check for Rails
            if (fs.existsSync(path.join(projectPath, 'config', 'application.rb'))) {
            ...
            if (fs.existsSync(gemfilePath)) {
                ...
                // Web frameworks
                if (gemfileContent.includes('gem "sinatra"') || gemfileContent.includes("gem 'sinatra'")) {return 'Sinatra';}
                if (gemfileContent.includes('gem "hanami"') || gemfileContent.includes("gem 'hanami'")) {return 'Hanami';}
                if (gemfileContent.includes('gem "roda"') || gemfileContent.includes("gem 'roda'")) {return 'Roda';}
                if (gemfileContent.includes('gem "grape"') || gemfileContent.includes("gem 'grape'")) {return 'Grape API';}
                if (gemfileContent.includes('gem "cuba"') || gemfileContent.includes("gem 'cuba'")) {return 'Cuba';}
                if (gemfileContent.includes('gem "padrino"') || gemfileContent.includes("gem 'padrino'")) {return 'Padrino';}
                
                // Testing frameworks
                if (gemfileContent.includes('gem "rspec"') || gemfileContent.includes("gem 'rspec'")) {return 'RSpec';}
                if (gemfileContent.includes('gem "minitest"') || gemfileContent.includes("gem 'minitest'")) {return 'Minitest';}
                
                // Background job frameworks
                if (gemfileContent.includes('gem "sidekiq"') || gemfileContent.includes("gem 'sidekiq'")) {return 'Sidekiq';}
                if (gemfileContent.includes('gem "resque"') || gemfileContent.includes("gem 'resque'")) {return 'Resque';}
                
                // CLI frameworks
                if (gemfileContent.includes('gem "thor"') || gemfileContent.includes("gem 'thor'")) {return 'Thor CLI';}
                if (gemfileContent.includes('gem "gli"') || gemfileContent.includes("gem 'gli'")) {return 'GLI CLI';}
                
                // Game frameworks
                if (gemfileContent.includes('gem "gosu"') || gemfileContent.includes("gem 'gosu'")) {return 'Gosu Game';}
            }
            ...
            if (fs.existsSync(path.join(projectPath, 'config.ru'))) {
                ...
                if (configRuContent.includes('Sinatra')) {return 'Sinatra';}
                if (configRuContent.includes('Hanami')) {return 'Hanami';}
                if (configRuContent.includes('Roda')) {return 'Roda';}
                return 'Rack Application';
            }

            // Check for specific file patterns
            if (fs.existsSync(path.join(projectPath, 'app.rb'))) {return 'Sinatra';}
            if (fs.existsSync(path.join(projectPath, 'Rakefile'))) {return 'Rake';}
    ...
    private async findRubyMainFiles(projectPath: string): Promise<string[]> {
        try {
            const mainFiles: string[] = [];
            
            // Search for Ruby entry points
            const searchForRubyFiles = (dir: string, relativePath = ''): void => {
                try {
                    const files = fs.readdirSync(dir);
    ...
    private hasRubyMainCode(rubyContent: string): boolean {
        // Remove comments first
        const cleanContent = this.removeRubyComments(rubyContent);
        
        // Look for main execution patterns
        const mainPatterns = [
            /if\s+__FILE__\s*==\s*\$0/,           // if __FILE__ == $0
            /if\s+\$0\s*==\s*__FILE__/,           // if $0 == __FILE__
            /if\s+\$PROGRAM_NAME\s*==\s*__FILE__/, // if $PROGRAM_NAME == __FILE__
            /Sinatra::Base/,                       // Sinatra app
            /class.*< Sinatra::Base/,              // Sinatra class
            /get\s+['"][^'"]*['"].*do/,            // Sinatra routes
            /post\s+['"][^'"]*['"].*do/,
            /put\s+['"][^'"]*['"].*do/,
            /delete\s+['"][^'"]*['"].*do/,
    ...
    private removeRubyComments(rubyContent: string): string {
        // Remove single-line comments (#)
    ...
    private async getSmartRubyCommands(
        framework: string,
        projectPath: string,
        mainFiles: string[],
        readmeInstructions: string[],
    ...
    private getDefaultRubyPort(framework: string): number {
        switch (framework) {
            case 'Ruby on Rails':
            case 'Rails':
            case 'Padrino':
    ...
    private async analyzeRubyDependencies(projectPath: string): Promise<any> {
        try {
            const results: any = {
                language: 'Ruby',
                dependencies: [],
            ...
            if (fs.existsSync(path.join(projectPath, 'Gemfile'))) {
                ...
                try {
                    ...
                    const bundleLines = stdout.split('\n').filter(line => 
                        line.includes('*') && !line.includes('gems included')
                    );
                ...
                try {
                    ...
                    const outdatedLines = outdatedOutput.split('\n').filter(line => 
                        line.includes('*') && line.includes('(newest')
                    );
                    results.outdated = outdatedLines.length;
    ...
    private async detectPHPFramework(projectPath: string): Promise<string> {
        try {
            // Check for Laravel
            ...
            if (fs.existsSync(composerPath)) {
                ...
                if (composerData.require) {
                    ...
                    if (deps['codeigniter4/framework'] || deps['codeigniter/framework']) {return 'CodeIgniter';}
                    if (deps['cakephp/cakephp']) {return 'CakePHP';}
                    if (deps['zendframework/zendframework'] || deps['laminas/laminas-mvc']) {return 'Laminas/Zend';}
                    if (deps['yiisoft/yii2'] || deps['yiisoft/yii']) {return 'Yii';}
                    if (deps['phalcon/phalcon']) {return 'Phalcon';}
                    if (deps['slim/slim']) {return 'Slim Framework';}
                    if (deps['lumen/lumen'] || deps['laravel/lumen-framework']) {return 'Lumen';}
                    
                    // Micro frameworks
                    if (deps['silex/silex']) {return 'Silex';}
                    if (deps['flight/flight']) {return 'Flight';}
                    if (deps['klein/klein']) {return 'Klein';}
                    
                    // CMS
                    if (deps['wordpress/wordpress'] || deps['johnpbloch/wordpress']) {return 'WordPress';}
                    if (deps['drupal/core'] || deps['drupal/drupal']) {return 'Drupal';}
                    if (deps['joomla/joomla-cms']) {return 'Joomla';}
                    
                    // Testing frameworks
                    if (deps['phpunit/phpunit']) {return 'PHPUnit';}
                    if (deps['codeception/codeception']) {return 'Codeception';}
                    if (deps['behat/behat']) {return 'Behat';}
                }
            }
            ...
            if (fs.existsSync(path.join(projectPath, 'index.php'))) {
                ...
                if (indexContent.includes('Laravel')) {return 'Laravel';}
                if (indexContent.includes('Symfony')) {return 'Symfony';}
                if (indexContent.includes('CodeIgniter')) {return 'CodeIgniter';}
                if (indexContent.includes('CakePHP')) {return 'CakePHP';}
            }
    ...
    private async findPHPMainFiles(projectPath: string): Promise<string[]> {
        try {
            const mainFiles: string[] = [];
            
            // Search for PHP entry points
            const searchForPHPFiles = (dir: string, relativePath = ''): void => {
                try {
                    const files = fs.readdirSync(dir);
    ...
    private hasPHPMainCode(phpContent: string): boolean {
        // Remove comments first
        ...
        const mainPatterns = [
            /<\?php/,                           // PHP opening tag
            /require_once.*bootstrap/,          // Bootstrap require
            /require_once.*autoload/,           // Autoload require
            /include.*config/,                  // Config include
            /new\s+Application/,                // Application instantiation
            /\$app\s*=\s*new/,                 // App variable
            /Router::/,                         // Router usage
            /Route::/,                          // Route usage
            /\$_GET\[/,                        // GET parameter handling
            /\$_POST\[/,                       // POST parameter handling
            /header\s*\(/,                     // Header function
    ...
    private removePHPComments(phpContent: string): string {
        // Remove single-line comments (//)
    ...
    private async getSmartPHPCommands(
        framework: string,
        projectPath: string,
        mainFiles: string[],
        readmeInstructions: string[],
    ...
    private getDefaultPHPPort(framework: string): number {
        switch (framework) {
            case 'Laravel':
            case 'Lumen':
            case 'Symfony':
    ...
    private async analyzePHPDependencies(projectPath: string): Promise<any> {
        try {
            const results: any = {
                language: 'PHP',
                dependencies: [],
