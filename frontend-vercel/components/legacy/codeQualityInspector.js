...
exports.AICodeQualityInspector = void 0;
const vscode = __importStar(require("vscode"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
class AICodeQualityInspector {
    constructor() {
        this.currentScore = null;
        this.issues = [];
        this.onQualityUpdatedEmitter = new vscode.EventEmitter();
        this.onQualityUpdated = this.onQualityUpdatedEmitter.event;
    ...
    async analyzeSecurityVulnerabilities(content, language, filePath) {
        const issues = [];
    ...
    async analyzePerformanceIssues(content, language, filePath) {
        const issues = [];
        const lines = content.split('\n');
    ...
    async analyzeMaintainabilityIssues(content, language, filePath) {
        const issues = [];
        const lines = content.split('\n');
    ...
    async analyzeReliabilityIssues(content, language, filePath) {
        const issues = [];
    ...
    getPerformancePatterns(language) {
        const patterns = {
            javascript: [
                {
                    regex: /for\s*\(.*\.length.*\)/,
    ...
    analyzeFunctionLength(content, language) {
    ...
    analyzeNamingConventions(content, language) {
    ...
    analyzeNullSafety(content, language) {
    ...
    convertToDiagnostics(issues) {
        return issues.map(issue => {
            const range = new vscode.Range(issue.line - 1, issue.column - 1, issue.line - 1, issue.column + 10);
            const diagnostic = new vscode.Diagnostic(range, `${issue.title}: ${issue.description}`, this.severityToDiagnosticSeverity(issue.severity));
    ...
    async getSourceFiles(workspacePath) {
        const files = [];
...
exports.AICodeQualityInspector = AICodeQualityInspector;
//# sourceMappingURL=codeQualityInspector.js.map