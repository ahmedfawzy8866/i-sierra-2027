...
export function useLeila() {
  const [stage, setStage] = useState<LeilaStage>('WELCOME');
  const [messages, setMessages] = useState<LeilaMessage[]>([]);
  ...
  const addLeilaMessage = (text: string, options?: string[], field?: string) => {
    setIsTyping(true);
    setTimeout(() => {
      const newMessage: LeilaMessage = {
  ...
  useEffect(() => {
    if (messages.length === 0) {
      addLeilaMessage(
        "Welcome to Sierra AI. I am Leila, your Master AI Concierge. Shall we initiate a search for your next high-yield asset?",
    ...
    switch (currentStage) {
      case 'WELCOME':
        if (text.toLowerCase().includes('initiate') || optionValue === 'Initiate Search') {
          ...
          addLeilaMessage("Understood. I am here if you decide to proceed with a strategic acquisition.");
        }
        break;
      ...
      case 'CONTACT':
        ...
        try {
          ...
          addLeilaMessage("I encountered a synchronization error, but our team has been notified.");
        }
        break;
