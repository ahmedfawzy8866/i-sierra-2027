...
export interface ConciergeSelection {
  ...
  createdAt: Timestamp;
  units: ConciergeUnit[];
  personalNote: string; // AI-written message from "Leila"
  whatsappLink: string;
 ...
 * This is the "Leila" magic moment — personal, editorial, exclusive.
 */
export async function curateConciergePortfolio(leadId: string): Promise<ConciergeSelection> {
  ...
  // 4. Generate personalized "Leila" note
  const personalNote = await generateLeilaNoteFromTemplate(lead, conciergeUnits);

  // 5. Create Concierge Selection document
  const portfolioRef = await addDoc(collection(db, COLLECTIONS.conciergeSelections || 'concierge_selections'), {
    leadId,
    leadName: lead.name,
...
/**
 * Generate a luxury property description in Leila's voice.
 */
async function generateLuxuryDescription(lead: Lead, unit: Unit): Promise<string> {
  const prompt = `You are Leila, a luxury property concierge for Sierra AI Realty. 
...
/**
 * Generate personalized "Leila" welcome note for the Concierge Selection.
 */
async function generateLeilaNoteFromTemplate(lead: Lead, units: ConciergeUnit[]): Promise<string> {
  const unitTitles = units.map(u => u.title).join(', ');
  
  const prompt = `You are Leila, the personal concierge at Sierra AI Realty.
...
Investment Goal: ${lead.investmentGoal || 'Long-term wealth creation'}
...
export async function trackPortfolioEngagement(
  ...
  await updateDoc(doc(db, COLLECTIONS.conciergeSelections || 'concierge_selections', portfolioId), {
