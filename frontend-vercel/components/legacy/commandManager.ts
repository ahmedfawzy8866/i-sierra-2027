...
export class CommandManager {
    ...
    async quickRestart() {
        try {
            ...
            vscode.window.showInformationMessage(
                `${projectInfo.framework} server restarted successfully!`
            );
    ...
    async detectProject() {
        ...
        if (projectInfo) {
            vscode.window.showInformationMessage(
                `Detected ${projectInfo.framework} project using ${projectInfo.packageManager}`
            );
        } else {
...
• Recommendations: ${analysis.recommendations.length}`;
    ...
    async analyzeError() {
        ...
        const panel = vscode.window.createWebviewPanel(
            'autoboot.errorAnalysis',
            'AutoBoot Error Analysis',
    ...
    private getErrorSuggestions(errorText: string): string[] {
        const suggestions = [];
    ...
    private getErrorAnalysisHtml(errorText: string, suggestions: string[]): string {
        return `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    async createProject() {
        ...
        vscode.window.showInformationMessage(
            `Creating ${framework} project "${projectName}" - this would use our scaffolding system!`
        );
    }
    ...
    private getDashboardHtml(projectInfo: any): string {
        return `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    async chooseRunOption() {
        ...
        if (language === 'Java' && projectInfo.mainClasses && projectInfo.mainClasses.length > 0) {
            projectInfo.mainClasses.forEach((mainClass: string, index: number) => {
        ...
        if (projectInfo.mainFiles && projectInfo.mainFiles.length > 0) {
            projectInfo.mainFiles.forEach((mainFile: string, index: number) => {
    ...
    private getMainFileCommand(language: string, packageManager: string, mainFile: string): string | null {
        switch (language) {
            case 'Go':
    ...
    private getFrameworkSpecificOptions(language: string, framework: string, projectInfo: any): vscode.QuickPickItem[] {
        const options: vscode.QuickPickItem[] = [];
