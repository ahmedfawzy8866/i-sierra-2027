...
export async function handleTelegramCommand(command: string, args: string[], chatId: string) {
  switch (command) {
    ...
    case 'start':
      return await sendTelegramMessage("Welcome to the <b>Sierra AI Intelligence Platform</b>. Send me a Signature Asset ID to analyze.", chatId);
    default:
...
async function cmdScore(unitId: string, chatId: string) {
  ...
  const text = `💎 <b>Signature Asset: ${unit.title}</b>\n\n` +
               ...
               `<i>Analysis powered by Sierra AI OS</i>`;
