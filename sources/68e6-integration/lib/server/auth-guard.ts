 ...
 * Checks the Firebase token AND verifies `role: 'admin'` (or `superadmin`)
 * in the Firestore `users` collection. Ported from the unified production
 * backend for use by privileged admin API routes.
 */
